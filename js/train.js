$(function() {
	$(".searchradio>label").click(function() {
		$(".searchradio>label").eq($(this).index()).css("color", "#00c0ff").siblings().css("color", "#585858");
	});
});
...
$(function() {
	for(var i = 0; i < pro.length; i++) {
		...
		$option.text(pro[i]);
		$(".trainfrom .province").append($option);
	};
	$(".trainfrom>img").click(function() {
		event.stopPropagation();
		$(".trainfrom .province").css("display", "block");
	});
	$(".trainfrom .province>li").click(function() {
		event.stopPropagation();
		$(".trainfrom>input").val($(this).text());
		$(".trainfrom .city").css("display", "block");
		$(".trainfrom .city>li").empty();
		var cities = city[$(this).text()];
		for(var i = 0; i < cities.length; i++) {
			$option = $("<li></li>");
			$option.attr("value", cities[i]);
			$option.text(cities[i]);
			$(".trainfrom .city").append($option);
		};
		$(".trainfrom .city>li").click(function() {
			var txt = $(".trainfrom>input").val();
			$(".trainfrom>input").val(txt + " / " + $(this).text())
			$(".trainfrom .province").css("display", "none");
			$(".trainfrom .city").css("display", "none");
		});
	});
...
$(function() {
	for(var i = 0; i < pro.length; i++) {
		...
		$option.attr("value", pro[i]);
		$option.text(pro[i]);
		$(".trainto .province").append($option);
	};
	$(".trainto>img").click(function() {
		event.stopPropagation();
		$(".trainto .province").css("display", "block");
	});
	$(".trainto .province>li").click(function() {
		event.stopPropagation();
		$(".trainto>input").val($(this).text());
		$(".trainto .city").css("display", "block");
		$(".trainto .city>li").empty();
		var cities = city[$(this).text()]
		for(var i = 0; i < cities.length; i++) {
			$option = $("<li></li>");
			$option.attr("value", cities[i]);
			$option.text(cities[i]);
			$(".trainto .city").append($option);
		};
		$(".trainto .city>li").click(function() {
			var txt = $(".trainto>input").val();
			$(".trainto>input").val(txt + " / " + $(this).text());
			$(".trainto .province").css("display", "none");
			$(".trainto .city").css("display", "none");
		});
	});
	$(document).click(function(event){
				$(".trainto .province").css("display", "none");
...
$(function() {
	$(".searchbody>img").click(function() {
		var txtex1 = $("#trainfrom").val();
		var txtex2 = $("#trainto").val();
		$("#trainto").val(txtex1);
		$("#trainfrom").val(txtex2);
	});
});
$(function() {
...
$(function() {
	$(".searchbody>label").click(function() {
		if($("#trainselect").is(':checked')) {
			$(".searchbody>label>div>div").css("background", "#00c0ff");
		} else {
			$(".searchbody>label>div>div").css("background", "#ffffff");
		}
